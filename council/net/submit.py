...
def main(persist: bool = False) -> int:
    """`persist=False` (default, `python -m council.net.submit`): unchanged legacy behavior —
    mints a fresh user each run and never saves the secret. `persist=True` (`pw ask`, R23
    ...
    if not user_secret and persist:
        ...
        if cached:
            ...
            # update the pointer — otherwise `pw fetch`/`pw rate` (which read "default" when
    ...
    if persist and minted:
        ...
        print(f"✓ new asker identity '{handle}' saved to {_asker_state_path()} (0600) — reused "
              f"automatically by `pw ask`/`pw fetch`/`pw rate` for {base}.\n  secret: {user_secret}\n")
...
def ask_main() -> int:
    """`pw ask "<brief>"` (R23 review): the asker front door. Same engine as
    `python -m council.net.submit`, but persists+prints the minted secret so it doesn't
    evaporate at process exit (F6) — `pw fetch`/`pw rate` read it back automatically."""

...
# RERANK=120 (council/rerank.py's web-evidence reranker, default-on for any non-time-sensitive
...
def _probe_tokens_per_sec(model: str, base_url: str, num_predict: int = 64,
    ...
    None on any failure — never raises into `pw status`."""
    import requests
    try:
...
def main(argv: list | None = None) -> int:
    ...
    # `pw status` stays the documented ~1-second preflight — this genuinely costs a few seconds.
    if eta:

#!/usr/bin/env python3
"""
council/config.py — `pw config`: persist your settings once, instead of re-exporting env vars
=============================================================================================
...
command (``pw config``) to read/write it.

The keystone is :func:`apply_to_env`, called as the FIRST thing in ``council.cli.main`` (before
...
bearer credentials), mirroring ``council.net.agent._save_join`` and ``council.crypto``. Secret
values are masked in ``pw config list`` and never logged. This module is intentionally stdlib-only
and import-light — it runs on every ``pw`` invocation.
"""
...
# `pw config list` shows and offers typo-correction against. `set` still accepts any other
...
KNOWN: dict[str, dict] = {
    ...
    "PW_EDITOR_MODEL":      {"help": "model for `pw research --editor api` (default openai/gpt-5-chat)", "secret": False},
...
def _save(state: dict) -> None:
    ...
    must never have a world-readable window. Same atomic pattern as council.net.agent._save_join."""
    p = path()
...
def set(key: str, value: str) -> None:  # noqa: A001 — deliberately named for the `pw config set` verb
...
def is_wellformed(key: str) -> bool:
    ...
    lowercase/typo'd/whitespace keys (so ``pw config set ollama_base …`` errors helpfully) while
...
_USAGE = (
    "usage:\n"
    "  pw config [list]              show current settings (secrets masked)\n"
    "  pw config get <KEY>           print one value\n"
    "  pw config set <KEY> <VALUE>   persist a setting\n"
    "  pw config unset <KEY>         remove a setting\n"
    "  pw config path                print the config file location"
)
...
def _cmd_list() -> int:
    ...
    if state:
        ...
        print(f"No config set yet — {p} does not exist.\n"
              f"Set one with e.g. `pw config set PW_WEB_BACKEND ddgs`.")
...
def main(argv: list | None = None) -> int:
    ...
    if verb == "set":
        if len(args) < 3:
            print("usage: pw config set <KEY> <VALUE>")
            return 2
        key = args[1]
        value = " ".join(args[2:])          # forgiving: tolerate an unquoted multi-word value
        if not is_wellformed(key):
            print(f"invalid key name: {key!r}{suggest(key)}\n"
                  f"keys are UPPER_SNAKE_CASE — see `pw config list` for recognized settings")
            return 2
        set(key, value)
        note = "" if key in KNOWN else "  (not a recognized setting — stored anyway; typo? see `pw config list`)"
        print(f"set {key} = {_display(key, value)}{note}")
        return 0

    if verb == "unset":
        if len(args) < 2:
            print("usage: pw config unset <KEY>")
            return 2
        key = args[1]

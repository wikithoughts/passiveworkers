...
python -m council.net.coordinator_app >/tmp/pw/coord.log 2>&1 & pids+=($!)
...
start_agent(){ # name owner country model lens can_judge judge_model
  ...
  python -m council.net.agent >"/tmp/pw/agent_$1.log" 2>&1 & pids+=($!)
}
...
echo ""
python -m council.net.submit --asker alice "What are two practical ways a small team can cut cloud costs, and one risk of each?"

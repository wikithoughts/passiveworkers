...
code in `council.operator` / `council.net.submit` (which call `requests.get/post`) run against the REAL
...
def coord_client(tmp_path, monkeypatch):
    ...
    for m in ("council.ledger", "council.net.config", "council.net.store", "council.net.coordinator_app"):

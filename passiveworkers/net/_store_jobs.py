#!/usr/bin/env python3
"""passiveworkers/net/_store_jobs.py — Store's job/task orchestration methods.

Extracted from passiveworkers/net/store.py (R37, step 4/4): job creation and worker/shard
selection, task claim/progress/completion, judging, settlement, and the D32 per-task
failover reaper. The single most correctness-critical group in Store — see `_settle`
below for the fail-closed settlement ordering this project depends on.

Uses `self._create_assisted` / `self._maybe_chain` (from `_AssistedMixin`) and
`self.online_nodes` / `self._save_ledger` (from `_LedgerMixin`) — all composed
alongside this mixin on the final `Store` class (never on this mixin alone).
...
from passiveworkers.ledger import InsufficientCredit
from passiveworkers.net._store_base import (
    _CLAIM_TIMEOUT_FRAC,
    _MAX_TASK_RETRIES,
    _StoreProtocol,
    _clip,
...
from passiveworkers.net.config import CONFIG, JOB_TYPES, pool_for, task_behavior
    ...
    def _settle(self, job_id: str, judge_owner: str, result: dict) -> None:
        ...
        if task_behavior(job["type"]).assemble == "shards":
            ...
            if job["as_file"]:
                ...
                # deliver a manifest the asker fetches+verifies as a single signed file (pworkers fetch).
                ...
                from passiveworkers.artifacts import chunk_bytes, wrap_artifact

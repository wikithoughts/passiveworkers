#!/usr/bin/env python3
"""passiveworkers/net/_store_base.py — shared helpers/constants for the Store mixins.

Kept as its own module (rather than living in passiveworkers/net/store.py) to avoid a
...
`from passiveworkers.net.store import ...` would try to import a module that is still
mid-import (store.py imports the mixins near its own top).

store.py re-imports `_hash` from here (`from passiveworkers.net._store_base import _hash as
_hash`) so `from passiveworkers.net.store import _hash` — used directly by
...
if TYPE_CHECKING:
    ...
    class _StoreProtocol:
        ...
        class (passiveworkers/net/store.py's `__init__` sets `lock`/`conn`/`ledger`/`_stop`; the

#!/usr/bin/env python3
"""
passiveworkers/net/agent.py — the networked worker daemon
==================================================
...
Run:  python -m passiveworkers.net.agent
"""
...
from passiveworkers.worker import Answer, PerspectiveWorker
...
class Agent:
    def __init__(self):
        ...
        self.enroll_token = _env("PW_ENROLL_TOKEN", "")   # D42: per-operator token for `pworkers join`
        self.node_id: str | None = None
        self.node_secret: str | None = None
        # D42: a callback (node_id, secret) -> None, set by `pworkers join` so a freshly minted identity is
    ...
    def _do_answer(self, task: dict) -> dict:
        ...
        if beh.executor == "research" \
            ...
            # research with citations (passiveworkers/researcher.py).
        ...
        if os.environ.get("PW_WEB_BACKEND", "off") != "off":
            try:
                from passiveworkers.research import search as web   # egress-localized web research
            except Exception as exc:
    ...
    def run(self) -> None:
        # D42 resume: a `pworkers join` that already has a cached identity skips the initial register
...
def _save_join(state: dict) -> None:
    ...
    (mirrors passiveworkers.crypto.load_or_create; no world-readable window before chmod)."""
    p = _join_state_path()
...
def _parse_join_args(rest: list) -> tuple[list, dict]:
    """Split `pworkers join` args into positionals (url, token) and flags
...
def join(argv: list) -> int:
    """`pworkers join <coordinator-url> <enrollment-token>` (first run) or `pworkers join` / `pworkers work` (resume):
    ...
    if not url:                                   # resume: fall back to the last coordinator joined
        ...
        if not url:
            raise SystemExit("usage: pworkers join <coordinator-url> <enrollment-token>   (first time)\n"
                             "       pworkers join | pworkers work                              (resume)")
    ...
    # deployment fails every job with "no judge node online" (found dogfooding pworkers join on the VPS).
    ...
    if installed is not None:
        ...
        if missing:
            ...
            raise SystemExit(
                f"✗ model(s) not pulled: {', '.join(missing)}\nPull them, then re-run `pworkers join`:\n"

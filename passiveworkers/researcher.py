#!/usr/bin/env python3
"""
passiveworkers/researcher.py — the iterative per-country researcher (D13, the pivot)
==============================================================================
The worker-side engine of the `research_report` job type. This node's OWN agent runs
multiple rounds of egress-localized web research (passiveworkers/research.py — the moat) and
...
from passiveworkers.research import (extract_date_hint, fetch_extract, inject_recency, is_breaking,
...
def _num_env(key: str, default, cast):
    ...
    (mirrors passiveworkers.ollama.resolve_timeout — an empty `PW_RESEARCH_DEADLINE=` must not raise). R36 review."""
...
class ResearchWorker:
    ...
    def _repair_draft(self, brief: str, draft: str, evidence: list[dict],
        ...
        from passiveworkers import fidelity
    ...
    def research(self, brief: str, on_progress: Callable[[str], None] | None = None) -> dict:
        # R26: stage-level progress for a caller that wants it (passiveworkers/local.py's per-analyst
        ...
        # passiveworkers.local's own _make_emit — a broken progress sink must never break a run.

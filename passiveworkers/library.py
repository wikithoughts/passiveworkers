#!/usr/bin/env python3
"""
passiveworkers/library.py — your private document library (local RAG, D19)
====================================================================
...
CLI:
    pworkers library add <path|dir>      index a file or a whole directory
    pworkers library list                what's indexed
    pworkers library remove <path>       drop a document
    pworkers library clear               wipe the library

Used by passiveworkers/researcher.py (the `[L#]` local citations) and the MCP server.
"""
...
class Library:
    ...
    def _corpus(self):
        ...
        import numpy as np
        from passiveworkers.retrieval import BM25Okapi, tokenize
        ...
        if rows and any(r["context"] is None for r in rows) and any(r["context"] for r in rows):
            print("  note: library mixes pre/post-contextual rows — `pworkers library clear` then "
    ...
    def search(self, query: str, k: int = 5, window: bool = True) -> list[dict]:
        ...
        try:
            import numpy as np
            from passiveworkers.retrieval import reciprocal_rank_fusion
    ...
    def _rerank(self, query: str, rows, ids: list[int], k: int) -> list[int]:
        ...
        candidates by actual relevance. Delegates to the shared passiveworkers.rerank reranker
        ...
        from passiveworkers.rerank import rerank_listwise
...
def main() -> int:
    ...
    if not args or args[0] == "list":
        srcs = lib.sources()
        if not srcs:
            print("library empty — add files with: pworkers library add <path>")
        else:
            for s in srcs:
    ...
    if cmd == "add":
        if len(args) < 2:
            print("usage: pworkers library add <path|dir>"); return 2
        try:
            n = lib.add(args[1])
    ...
    if cmd == "remove":
        if len(args) < 2:
            print("usage: pworkers library remove <path>"); return 2
    ...
    if cmd == "search":
        if len(args) < 2:
            print("usage: pworkers library search <query>"); return 2
        if lib.is_empty():
            print("library empty — add files with: pworkers library add <path>"); return 0
        ...
        return 0
    print(f"unknown: pworkers library {cmd}  (add | list | search | remove | clear)")
    return 2

#!/usr/bin/env python3
"""
passiveworkers/local.py — single-player deep research, one command (D16)
==================================================================
    python -m passiveworkers.local "your brief"            # or: pworkers research "your brief"
    python -m passiveworkers.local "brief" --quick|--deep
    python -m passiveworkers.local "brief" --editor api    # BYOK frontier editor over LOCAL findings
...
The network (passiveworkers/net/) is the optional multiplayer mode — this command needs none of it.
"""
...
from passiveworkers.judge import Judge, _drop_invented_markers, _extract_json
...
from passiveworkers import research as _research
...
def run(brief: str, depth: str = "standard", editor_mode: str = "local",
    ...
    if total_src == 0:
        raise SystemExit(
            ...
            "this run (try `pworkers config set PW_WEB_BACKEND searxng` or a keyed backend), your network "
            "is down, or (with --local) no library document matched — `pworkers library add <path>` "
    ...
    if as_html:
        from passiveworkers import render
...
def main() -> int:
    p = argparse.ArgumentParser(prog="pworkers research",
    ...
    try:
        ...
        return 130
    if a.json:
        # progress goes to stderr, so stdout is clean → `pworkers research "…" --json > out.json` works

#!/usr/bin/env python3
"""
passiveworkers/trust.py — out-of-band operator key trust (D25, FEDERATION_V2)
=====================================================================
...
operator's signing key and verifies every later deliverable against the PINNED key.
  ...
  • Explicit pin — `pworkers trust add <operator> <pubkey>`: the operator shares their PUBLIC signing
...
def classify(operator: str, presented_pub: str) -> tuple[str, dict | None]:
    ...
    Pinning a rotated key is always an explicit, human-verified action (`pworkers trust add`), never

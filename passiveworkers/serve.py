#!/usr/bin/env python3
"""
passiveworkers/serve.py — the local research desk (single-user UI for passiveworkers.local)
==============================================================================
    python -m passiveworkers.serve          # → http://127.0.0.1:8770
    pworkers serve
...
report in ./reports/. The network app (passiveworkers/net) is the separate multiplayer mode.
"""
...
from passiveworkers.local import Cancelled, run as run_research
from passiveworkers import paths

app = FastAPI(title="Passive Workers — local research desk")
REPORTS = paths.reports_dir()   # shared ~/.passiveworkers/reports (see passiveworkers/paths.py)
...
# Shared UI fragments (theme palette, footer, esc()), substituted in at import (passiveworkers.net.ui_common).

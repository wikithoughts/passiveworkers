#!/usr/bin/env python3
"""
passiveworkers/research.py — per-node, egress-localized web research (M4)
=================================================================
...
(the legal bright line; see passiveworkers/worker.py and docs/DECISIONS D4).
...
Config (per node, via env — persist with `pworkers config set …`):
...
Wire-in: passiveworkers/net/agent.py passes search() as PerspectiveWorker(web_search=…) when
...
def _backend() -> str:
    # Read at CALL time, not import time — callers (e.g. passiveworkers.local) may enable the
...
def reset_ddg_breaker() -> None:
    """Clear the DDG failure streak — call once per `pworkers research` run. Module-global state would
    otherwise leak one bad run's DDG outage into an unrelated later run in a long-lived process
    (e.g. `pworkers serve`) (R17 review)."""
...
def _ddgs(question: str) -> list[dict]:
    ...
    if breaker > 0 and _ddg_fail_streak >= breaker:
        # A long-lived process (the `pworkers join`/`pworkers work` agent daemon) never calls
        # reset_ddg_breaker() between tasks by design — only the one-shot `pworkers research` CLI does
...
def _searxng(question: str) -> list[dict]:
    ...
    # PW_SEARXNG_URL is OPERATOR-supplied trusted config (env var / `pworkers config set`) — never
...
# once with `pworkers config set …`). Each returns the common row shape {title, href/url, body/content}
...
def search_structured(query: str, max_results: int = 5, engine: str = "web") -> list[dict]:
    ...
    try:
        ...
        return []
    from passiveworkers.sanitize import clean as _sanitize
    out, seen = [], set()
    for r in rows:
        url = (r.get("href") or r.get("url") or "").strip()
        host = (urlparse(url).hostname or "").lower()
...
def fetch_extract(url: str, max_chars: int = 6000, with_date: bool = False):
    ...
    from passiveworkers.sanitize import clean
